 ...
 */
class NewUserShareAdapter(data: ArrayList<SchoolSourceBean>?) : BaseDelegateMultiAdapter<SchoolSourceBean, BaseViewHolder>(data) {
    var displayTypes = arrayOf(ResourceTypeConstans.TYPE_COURSE, ResourceTypeConstans.TYPE_TRACK,
            ResourceTypeConstans.TYPE_ALBUM, ResourceTypeConstans.TYPE_ARTICLE, ResourceTypeConstans.TYPE_PERIODICAL,
            ResourceTypeConstans.TYPE_E_BOOK, ResourceTypeConstans.TYPE_MICRO_LESSON, ResourceTypeConstans.TYPE_BAIKE,
...
//                    val dataBean: StudyDynamic = resultBean.detail_info as StudyDynamic
        ...
        getMultiTypeDelegate()
                ...
                ?.addItemType(ResourceTypeConstans.TYPE_FOLLOWUP_RESOURCE, R.layout.my_item_userzone_myshare_dynamic_followup)
                ?.addItemType(ResourceTypeConstans.TYPE_STUDY_PLAN_DYNAMIC, R.layout.my_item_userzone_myshare_dynamic)
                ?.addItemType(ResourceTypeConstans.TYPE_SPECIAL, R.layout.my_item_userinfo_myshare_special)
                ?.addItemType(ResourceTypeConstans.TYPE_UNDEFINE, R.layout.common_item_empty)
    }

    private fun goneBottom(holder: BaseViewHolder) {
        val view = holder.getViewOrNull<ConstraintLayout>(R.id.incloudBottom)
        if (view != null) {
            view.visiable(false)
        }
    }
...
//                setBottomData(holder.getView<ConstraintLayout>(R.id.incloudBottom), item, "专题")
...
//                setBottomData(holder.getView<ConstraintLayout>(R.id.incloudBottom), item, "")
...
//                setBottomData(holder.getView<ConstraintLayout>(R.id.incloudBottom), item, "百科")
            ...
            ResourceTypeConstans.TYPE_NOTE -> {
                ...
                goneBottom(holder)
//                setBottomData(holder.getView<ConstraintLayout>(R.id.incloudBottom), item, "笔记")
...
//                setStudyPlanData(holder, item)
                val binding = DataBindingUtil.bind<MyItemUserzoneMyshareDynamicFollowupBinding>(holder.itemView)
...
//                setBottomData(holder.getView<ConstraintLayout>(R.id.incloudBottom), item, "动态")
            ...
            ResourceTypeConstans.TYPE_STUDY_PLAN_DYNAMIC -> {
                holder.setGone(R.id.line,true)
                var binding = DataBindingUtil.bind<MyItemUserzoneMyshareDynamicBinding>(holder.itemView)
                val toJson = Gson().toJson(item.detail_info)
                val bean = Gson().fromJson(toJson, StudyDynamic::class.java)
...
//                setBottomData(holder.getView<ConstraintLayout>(R.id.incloudBottom), item, "动态")
    ...
    private fun setSpecialData(holder: BaseViewHolder, bean: SchoolSourceBean) {
        val gson = Gson()
        val toJson = gson.toJson(bean.detail_info)
        val item = gson.fromJson(toJson, SpecialDetailBean::class.java)
        holder.setText(R.id.tvTitle, item.title)
        holder.setText(R.id.tvContent, item.about)
    }
    ...
    private fun setNoteData(holder: BaseViewHolder, bean: SchoolSourceBean) {
        ...
        holder.setText(R.id.tvNoteBelong, context.resources.getString(R.string.string_vertical_bar) + item.recommend_title)
     ...
     * 设置底部数据展示
     */
    private fun setBottomData(layout: ConstraintLayout, item: SchoolSourceBean, lableString: String) {
        var binding = DataBindingUtil.bind<MyIncludeItemMyshareBottomBinding>(layout)
...
//        }
    ...
    private fun setCourseData(holder: BaseViewHolder, item: SchoolSourceBean) {
        ...
        var tvStaffStr = if (bean.staffsBeenList != null && (bean.staffsBeenList?.size?:0)> 0) bean.staffsBeenList?.get(0)?.name else ""
        if (bean.staffsBeenList != null && (bean.staffsBeenList?.size?:0)> 1) {
            tvStaffStr += "等"
        }
...
//            }
            tvCertificateType.text = context.getString(R.string.point) + context.resources.getString(R.string.course_str_have_certificate)

        } else {
            tvCertificateType.text = context.getString(R.string.point) + context.resources.getString(R.string.course_str_no_certificate)
        }
        tvCertificateType.setTextColor(context.resources.getColor(R.color.colorPrimary))

        if (isFree == "0") {
            tvPriceType.text = context.getString(R.string.point) + context.resources.getString(R.string.course_str_pay)
        } else {
            tvPriceType.text = context.getString(R.string.point) + context.resources.getString(R.string.course_str_free)
        }
     ...
     * @param dataBean
     */
    private fun setFolloupItem(itemStudyFirendCircleAlbumBinding: MyItemUserzoneMyshareDynamicFollowupBinding, dataBean: StudyDynamic) {
...
//            boolean expandTvVisibale = itemViewHolder.tvFollowUpAudioExpand.getVisibility() == View.GONE;
            if (repeat_record.size > 4) {   //如果超过6条，显示4条，并且显示展开view
                ...
                itemStudyFirendCircleAlbumBinding.incloudItem.tvFollowUpAudioExpand.setVisibility(View.VISIBLE)
                itemStudyFirendCircleAlbumBinding.incloudItem.tvFollowUpAudioExpand.setCompoundDrawables(null, null, getTvFollowupDrawableRight(itemStudyFirendCircleAlbumBinding), null)
                //                followUpVoiceDynamicAdapter.setExpand(false);
                val subList: java.util.ArrayList<StudyDynamic.RepeatRecordBean> = java.util.ArrayList<StudyDynamic.RepeatRecordBean>()
                for (i in 0..3) {   //截取前4个音频
                    subList.add(repeat_record[i])
                }
                followUpVoiceDynamicAdapter.setNewData(subList)
                itemStudyFirendCircleAlbumBinding.incloudItem.tvFollowUpAudioExpand.setOnClickListener(View.OnClickListener {
                    val equals = "展开" == itemStudyFirendCircleAlbumBinding.incloudItem.tvFollowUpAudioExpand.getText().toString()
                    ...
                    itemStudyFirendCircleAlbumBinding.incloudItem.tvFollowUpAudioExpand.setText(expandStr)
                    val tvFollowupDrawableRight = getTvFollowupDrawableRight(itemStudyFirendCircleAlbumBinding)
                    itemStudyFirendCircleAlbumBinding.incloudItem.tvFollowUpAudioExpand.setCompoundDrawables(null, null, tvFollowupDrawableRight, null)
                })
            } else {
                followUpVoiceDynamicAdapter.setNewData(repeat_record)
                itemStudyFirendCircleAlbumBinding.incloudItem.tvFollowUpAudioExpand.setVisibility(View.GONE)
            }
            itemStudyFirendCircleAlbumBinding.incloudItem.recyclerView.setAdapter(followUpVoiceDynamicAdapter)
            itemStudyFirendCircleAlbumBinding.incloudItem.recyclerView.setVisibility(View.VISIBLE)
        }
    }
    ...
    private fun getTvFollowupDrawableRight(itemViewHolder: MyItemUserzoneMyshareDynamicFollowupBinding): Drawable? {
        ...
        val drawRight: Int = if (equals) R.mipmap.common_ic_arrow_down_blue else R.mipmap.common_ic_arrow_up_blue
    ...
    private fun setTvSpan(tv: TextView, dataBean: StudyDynamic) {
        ...
        //资源标题
        val title: String = dataBean.source_title.toString()
        var str = ""

        //如果是跟读状态，和动态有所差别
        if (dataBean.source_type_id == ResourceTypeConstans.TYPE_FOLLOWUP_RESOURCE) {
            ...
            str += dataBean.publish_content
            spannableString = SpannableString(str)
            spannableString.setSpan(ForegroundColorSpan(context.getResources().getColor(R.color.colorPrimary)), 0, 6, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
            spannableString.setSpan(ForegroundColorSpan(context.getResources().getColor(R.color.color_4A90E2)), str.indexOf(title) - 1, str.indexOf(title) + title.length + 1, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)

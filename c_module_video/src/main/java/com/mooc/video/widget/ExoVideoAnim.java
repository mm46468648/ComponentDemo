...
import com.mooc.video.util.ParamsCreator;
...
public class ExoVideoAnim extends View {
    ...
    public ExoVideoAnim(@NonNull Context context, AttributeSet attrs) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.VideoLoadAnim);
        circleRadius = (int) a.getDimension(R.styleable.VideoLoadAnim_vla_circleRadius, paramsCreator.getDefaultCircleRadius());
        circleSpacing = (int) a.getDimension(R.styleable.VideoLoadAnim_vla_circleSpacing, paramsCreator.getDefaultCircleSpacing());
        //周期，默认为2秒
        int cycle = a.getInt(R.styleable.VideoLoadAnim_vla_cycle, 2000);
        cycle = cycle / 2;

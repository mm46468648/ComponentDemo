 ...
 * 发现页慕课列表
 */
class MicroKnowChildFragment : BaseListFragment2<MicroKnowBean, MicroKnowChildViewModel>(),DiscoverAcitivtyFloatView.FloatViewVisibale {


    var ordering = "-sort_top" //排序默认综合  -view_count：最热      -publish_time ：最新    -sort_top：综合
    var discoverMoocChildViewModel : MicroKnowChildViewModel? = null

    companion object {

        const val PARAMS_PARENT_ID = "params_parent_id"
        const val PARAMS_RESOURCE_TYPE = "params_resource_type"
        fun getInstance(bundle: Bundle? = null): MicroKnowChildFragment {
            val fragment = MicroKnowChildFragment()
            bundle?.apply {
    ...
    override fun initAdapter(): BaseQuickAdapter<MicroKnowBean, BaseViewHolder>? {
        return (mViewModel as MicroKnowChildViewModel).getPageData().value?.let {
            val adapter = MicroKnowAdapter(it)

            adapter.setOnItemClickListener { adapter, _, position ->
                val bean = adapter.data.get(position) as MicroKnowBean

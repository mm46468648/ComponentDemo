...
import com.mooc.commonbusiness.adapter.CommonSlightCourseAdapter
 ...
 * 发现页音频列表
 */
class SlightCourseChildFragment : BaseListFragment<MicroBean, SlightCourseChildViewModel>()
    ...
    var discoverAudioChildViewModel : SlightCourseChildViewModel? = null

    companion object {

        const val PARAMS_PARENT_ID = "params_parent_id"
        const val PARAMS_RESOURCE_TYPE = "params_resource_type"
        fun getInstance(bundle: Bundle? = null): SlightCourseChildFragment {
            val fragment = SlightCourseChildFragment()
            bundle?.apply {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        discoverAudioChildViewModel = mViewModel as SlightCourseChildViewModel
        ...
        //条数
        discoverAudioChildViewModel?.listCount?.observe(this, Observer {
            //子fragment 跟父fragment通信
            if(parentFragment!=null && parentFragment is SlightCourseFragment){
                (parentFragment as SlightCourseFragment).setNumStr(it?:"0")
            }
//            LiveDataBus.get().with(LiveDataBusEventConstants.EVENT_DISCOVER_CHILD_LIST_COUNT).postValue(it)
    ...
    override fun initAdapter(): BaseQuickAdapter<MicroBean, BaseViewHolder>? {
        return (mViewModel as SlightCourseChildViewModel).getPageData()?.value?.let {
//            val discoverMoocAdapter = SlightCourseAdapter(it)
            val discoverMoocAdapter = CommonSlightCourseAdapter(it)
    ...
    override fun changeListData(map:Map<String,String>,reset:Boolean){
        if(reset){    //当点击三级列表的时候需要重置，查询条件
            ...
            val pId = arguments?.getString(PARAMS_PARENT_ID, "") ?: ""
            val resourceType = arguments?.getString(PARAMS_RESOURCE_TYPE, "") ?: "-1"
            val discoverMoocChildViewModel = mViewModel as SlightCourseChildViewModel

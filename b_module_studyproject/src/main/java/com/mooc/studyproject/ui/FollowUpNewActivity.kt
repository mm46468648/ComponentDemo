...
@Route(path = Paths.PAGE_FOLLOWUP)
class FollowUpNewActivity : BaseActivity() {
    ...
    companion object {
        ...
        const val PARAMS_RESOURSE_ID = "params_resource_id" //跟读资源id
        const val PARAMS_CHECKIN_SOURCE_ID = "params_checkin_source_id" //学习资源id
        const val PARAMS_STUDYPLAN_ID = "params_studyplan_id" //学习计划id
...
//            checkRecordPermission()
    ...
    private fun showLastNoUploadDialog(text: String) {
        val lastMp3 = mOral.getLastMp3(resourseid + text);
...
//                    mOral.stopLocalOral()
        ...
        override fun onEvaluationError(data: TAIOralEvaluationData?, error: TAIError?) {
        }
...
//        mOral.saveMp3(resourseid + currentReadBean?.context)
        mOral.currenMp3="";
        mOral.deleteMp3Key(resourseid + currentReadBean?.context)
    }
...
//        val iSBindEbook = intent.getBooleanExtra(PARAMS_IS_BIND_EBOOK, false)
        ...
        val bindResourseid = intent.getStringExtra(PARAMS_RESOURSE_ID) ?: ""
        ...
        ApiService.getRetrofit().create(UserApi::class.java).getFollowupDetail(bindResourseid)
                .compose(RxUtils.applySchedulers())
                        ...
                        if ("1".equals(data.is_bind_source) && data.bind_source_id.isNotEmpty() && data.bind_source_type == ResourceTypeConstans.TYPE_E_BOOK) {
                            ...
                            ApiService.getRetrofit().create(UserApi::class.java)
                                    ...
                                    .compose(RxUtils.applySchedulers()).subscribe(object :
                                            BaseObserver<EBookBean>(this@FollowUpNewActivity) {
...
//        }
    ...
    fun requestReadContentList() {
        ...
        //获取跟读内容列表
        ApiService.getRetrofit().create(UserApi::class.java)
                .getFollowupResourseList(resourseid, checkin_resource_id)
                .compose(RxUtils.applySchedulers())
...
//        )
        ...
        if (value.isNotEmpty()) {
            ...
            val find = fromJson.find {
                it.id == lastFollowData.id + resourseid + lastFollowData.repeat_books_id
            }
...
//        uploadMp3Pop.setPorText("排队中...")
    ...
    private fun postAudioUrl(
        ...
        jsonObject.put("resource_id", checkin_resource_id)
...
//        )
        ...
        ApiService.getRetrofit().create(UserApi::class.java).postAudioUrl(create)
                .compose(RxUtils.applySchedulers())
...
//        )
        val create = RequestBodyUtil.fromJson(jsonObject)
        ApiService.getRetrofit().create(UserApi::class.java).postFollowUpAudioUrl(create)
                .compose(RxUtils.applySchedulers())
                .subscribe(object : BaseObserver<HttpResponse<Any>>(this, false) {
                    override fun onSuccess(t: HttpResponse<Any>?) {
...
//    var wait_time = 0
    ...
    private fun getRepeatIsOverLimit() {
        ...
        currentReadBean?.apply {
            ApiService.getRetrofit().create(UserApi::class.java)
                    .getRepeatAudioLimit(studyplanId, this.id, this.repeat_books_id)
                    .compose(RxUtils.applySchedulers())
                    .subscribe(object :
                            BaseObserver<HttpResponse<OverLimitBean>>(this@FollowUpNewActivity, false) {
    ...
    private fun delRepeatAudioLimit() {
        ...
        ApiService.getRetrofit().create(UserApi::class.java)
                .delRepeatAudioLimit(RequestBodyUtil.fromJson(jsonObject))
                .compose(RxUtils.applySchedulers())
                .subscribe(object : BaseObserver<HttpResponse<Any>>(this, false) {
                    override fun onSuccess(t: HttpResponse<Any>?) {
...
//        }
        ...
        val n2 = MultipartBody.Part.createFormData("studyplan_id", studyplanId);
        val n3 = MultipartBody.Part.createFormData(
                "repeat_books_context_id", repeat_books_context_id
                ?: ""
        );
        val n4 = MultipartBody.Part.createFormData(
                "repeat_books_id", currentReadBean?.repeat_books_id
                ?: ""
        );
        val n5 = MultipartBody.Part.createFormData("audio_time", audio_time);

        val list = listOf<MultipartBody.Part>(n1, n2, n3, n4, n5)


        ApiService.getRetrofit().create(UserApi::class.java).uploadRepeatAudio(list, requestBody)
                .compose(RxUtils.applySchedulers())
                .subscribe(object : BaseObserver<HttpResponse<FollowUrlBean>>(this, false) {
                    override fun onSuccess(t: HttpResponse<FollowUrlBean>?) {
                        ...
                        if (isUploadWait) {   //是服务器评测
                            this@FollowUpNewActivity.currentReadBean?.let {
                ...
                MESSAGE_LOOP -> {
                    ...
                    if (playTime > LOOPTIME && weakActivity.get() != null) {
                        currentFollowUpData?.let {
                            (weakActivity.get() as FollowUpNewActivity).checkBackTestSuccess(it)
                        }
                        return
                    }

 ...
 */
class CommentListNewAdapter(data: ArrayList<ItemComment>?) :
    BaseQuickAdapter<ItemComment, BaseViewHolder>(R.layout.studyproject_item_comment_list_new, data),
    LoadMoreModule {
...
//        }
        if (item.comment_type == 1) {            //音频评论
            ...
            if (!item.comment_content.isNullOrEmpty()) {
                ...
                holder.setGone(R.id.commentVoice, false)
                holder.getView<VoicePlayerController>(R.id.commentVoice)
                    .setTotleTimeLength(item.comment_content_long)
                holder.getView<VoicePlayerController>(R.id.commentVoice)
                    .setPlayPath(item.comment_content)
            } else {
        ...
        if (mStudyPlanDetailBean != null && mStudyPlanDetailBean.is_join == 0) {
            ...
            if (mIsInitiator) {
                ...
                if (item.comment_status == 0) {
                    holder.setText(
                        R.id.tvShied,
                        context.resources.getString(R.string.study_plan_cancel_stop)
                    )
                } else {
                    holder.setText(
                        R.id.tvShied,
                        context.resources.getString(R.string.study_plan_stop)
                    )
                }
        ...
        if (item.is_from_cms == 1) {
                ...
                if (item.is_studyplan_start_user) {
                    if (!TextUtils.isEmpty(commentUserBean.name)) {
                        name = java.lang.String.format(
                            context.getResources().getString(R.string.text_initiator),
            ...
            if (commentUserBean != null) {
                if (item.is_studyplan_start_user) {
                    if (!TextUtils.isEmpty(commentUserBean.name)) {
                        name = java.lang.String.format(context.getResources()
                            .getString(R.string.text_initiator),
                            commentUserBean.name?.let { getStrUserName(it) })
                    } else {
                        name = context.getResources().getString(R.string.text_str_initiator)
                    }
...
//            }
            ...
            //根据行数，判断是否显示展开
            {
                if (tvContent.getLineCount() > 2) {
                    holder.setGone(R.id.tvContentExpand, false)
//                tvContent.setTag(tvContent.lineCount)
                    tvContent.maxLines = 2
    ...
    private fun setCommentReplay(holder: BaseViewHolder, dataBean: ItemComment) {
        ...
        if (infoBean.comment_type.equals("1")) {
            if (!infoBean.comment_content.isNullOrEmpty()) {//音频未被删除
                ...
                (holder.getView<VoicePlayerController>(R.id.replyTrackRl)).setTotleTimeLength(
                    infoBean.comment_content_long
                )
                (holder.getView<VoicePlayerController>(R.id.replyTrackRl)).playPath =
                    infoBean.comment_content
            } else {
        ...
        holder.getView<com.mooc.resource.widget.expandabletextview.ExpandableTextView>(R.id.tvToReply)

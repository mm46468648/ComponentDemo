...
public class StarBar extends View {
    ...
    private boolean isAbleChange;

    @TargetApi(21)
    public StarBar(Context context, @Nullable AttributeSet attrs, int defStyleAttr, int defStyleRes) {
...
//        setClickable(true);
        ...
        this.starEmptyDrawable = (Drawable) mTypedArray.getDrawable(R.styleable.RatingBar_starEmpty);

        this.starFillBitmap = (Bitmap) drawableToBitmap(mTypedArray.getDrawable(R.styleable.RatingBar_starDrawable));
        mTypedArray.recycle();
    ...
    public void setAbleChange(boolean ableChange) {
        isAbleChange = ableChange;
    }
    ...
    public void setStarMark(float mark) {
        if (integerMark) {
            ...
            float starMarkUp = (int) Math.ceil(mark);//向上取整
            float starMarkDown = (int) Math.floor(mark);//向下取整
            starMark = Math.round(mark * 10) * 1.0f / 10;
            if (starMark-starMarkDown>starMarkUp-starMark) {//大于5取整
                starMark=starMarkUp;
            }else if (starMark-starMarkDown==starMarkUp-starMark){//相等

            }else{//可以取半
                starMark=starMarkUp-(float) 0.5;
            }

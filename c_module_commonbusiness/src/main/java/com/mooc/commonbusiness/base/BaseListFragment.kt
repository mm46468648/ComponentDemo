 ...
 * @param M ViewModle类型
 */
abstract class BaseListFragment<T, M : BaseListViewModel<T>> : BaseFragment() ,LifecycleObserver {
...
//    var refresh_layout: SwipeRefreshLayout? = null
    ...
    var moocPullRefresh: com.mooc.resource.widget.pullrefresh.MoocPullRefreshHeader? = null
    ...
    private fun initView(inflate: View) {
        ...
        flRoot = inflate.findViewById(R.id.flRoot)
        refresh_layout = inflate.findViewById(R.id.refreshLayout)
        moocPullRefresh = inflate.findViewById(R.id.moocPullRefresh)
...
//        refresh_layout?.isRefreshing = true
     ...
     * 刷新数据不刷新动画
     */
    open fun loadDataWithNoRrefresh() {
//        refresh_layout?.isRefreshing = true
        mViewModel?.initData()
    }
...
//    open fun needLogin() = false
    ...
    open fun genericViewModel() {
        ...
        if (arguments.size > 1) {
            ...
            val modelClaz = (argument as Class<M>).asSubclass(BaseListViewModel::class.java)

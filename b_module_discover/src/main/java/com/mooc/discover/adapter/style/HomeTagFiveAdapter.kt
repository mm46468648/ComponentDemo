...
import com.mooc.column.ui.columnall.delegate.ColumnTagFiveDelegate
...
class HomeTagFiveAdapter(data: ArrayList<RecommendColumn>?) :
     ...
     */
    init {
        setMultiTypeDelegate(ColumnTagFiveDelegate())
    }
    ...
    override fun convert(holder: BaseViewHolder, item: RecommendColumn) {
        ...
        holder.setText(R.id.tvTypeHomeFive, value)
        holder.setGone(R.id.tvTypeHomeFive, true)
        ...
        when (item.type) {
            ...
            ResourceTypeConstans.TYPE_COURSE -> {
                ...
                if (!TextUtils.isEmpty(item.identity_name)) { //后台返回的右上角角标不为空，显示后台配置
                    holder.setGone(R.id.tvTypeHomeFive, false)
                    holder.setGone(R.id.tv_identify, true)
                    holder.setText(R.id.tvTypeHomeFive, item.identity_name)
                } else {
                    holder.setGone(R.id.tvTypeHomeFive, true)
            ...
            ResourceTypeConstans.TYPE_TRACK -> {
                ...
                val (_, _, _, _, subordinated_album, _, play_count) = item.track_data
                if (subordinated_album != null) {
                    try {
                        holder.setText(
                            R.id.org, String.format(
                                "播放 %s次", formatPlayCount(
                                    play_count
                                )
                            )
                        )
            ...
            ResourceTypeConstans.TYPE_ARTICLE -> holder.setText(R.id.org, item.source)
            ResourceTypeConstans.TYPE_COLUMN_ARTICLE -> holder.setText(R.id.org, item.source)
...
//            ResourceTypeConstans.TYPE_ACTIVITY, ResourceTypeConstans.TYPE_ACTIVITY_TASK -> holder.setText(
//                R.id.org,
//                item.about
//            )
    ...
    private fun setBottomGone(helper: BaseViewHolder) {
        ...
        if (tvOrg != null && tvOrg is TextView) {

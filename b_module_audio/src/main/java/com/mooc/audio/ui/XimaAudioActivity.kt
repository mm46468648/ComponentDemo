...
import com.mooc.audio.viewmodel.XimaAudioPlayNewModel
...
open class XimaAudioActivity : BaseActivity(), XiMaUtile.StatusListener {
    ...
    val mViewModel: XimaAudioPlayNewModel by viewModels()
...
//    private var mPlayerManager: XmPlayerManager? = null
    ...
    private fun initData(data: Intent?) {
        ...
        if (!offLineModel) {
            mViewModel.getTrackFromServer(mViewModel.trackId)
        } else {
            mViewModel.getTracFromLocal(mViewModel.trackId)
        }
    ...
    var mTrackList: ArrayList<TrackBean>? = null
    var currentAlbumId:String = ""
    fun makeTrakList(albumId:String) {
        loge("发起专辑信息请求: ${albumId}")
        //id相同，并且列表有值返回
        if(albumId == currentAlbumId && mTrackList?.isNotEmpty() == true){
            hideProgressBar()
            return
        }

        loge("专辑id不同，新${albumId},旧${currentAlbumId},或者专辑列表为空 ${mTrackList}")

        //id不同，或者列表为空
        currentAlbumId = albumId

        lifecycleScope.launchWhenCreated {
            mViewModel.loadAlbumList(albumId)
                .catch {
                    loge("加载专辑异常${albumId}")
                }
                .collect {
                    loge("加载专辑成功")
                    mTrackList = it as ArrayList<TrackBean>
                    var findIndex = 0
                    //将这个专辑后面的插入到播放列表中
                    it.forEachIndexed { index, trackBean ->
                        trackBean.albumId = trackBean.subordinated_album?.id ?: ""
                        if (trackBean.id == mViewModel.trackId) {
                            findIndex = index
                        }
                    }

                    xiMaUtile?.setList(it, findIndex)
                    xiMaUtile?.playTrack(this@XimaAudioActivity)
                    hideProgressBar()
                }
        }
    }
...
//            mPresenter.showSettingPop(builder)
            inflater.commonTitle.ib_right?.let {
                    ibRight -> mPresenter.showSettingPop(ibRight,mViewModel.has_permission == "1") }
        })
...
//        mViewModel.postStudyLog(trackModel)
        ...
        FirstAddPointManager.startAddFirstPointsTask(
            this, ResourceTypeConstans.typeAliasName[ResourceTypeConstans.TYPE_TRACK]
                ?: "", trackModel.id
        )
        ...
        //专辑id不为空，创建播放列表
        if (trackModel.subordinated_album?.id?.isNotEmpty() == true) {
            makeTrakList(trackModel.subordinated_album?.id?:"")
        }else{
            //如果专辑id空将单挑音频传递播放
            xiMaUtile?.setList(arrayListOf(trackModel) as List<BaseAudioModle>?, 0)
            xiMaUtile?.playTrack(this@XimaAudioActivity)
            hideProgressBar()
        }
    }

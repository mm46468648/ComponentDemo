...
class FlowLayout<T> : ViewGroup {
     ...
     * 数据适配器
     */
    private var mAdapter: com.mooc.resource.widget.flowLayout.FlowAdapter<T>? = null
    ...
    private fun init(context: Context, attrs: AttributeSet) {
        ...
        mCutLineColor = ta.getColor(R.styleable.AutoFlowLayout_cutLineColor, resources.getColor(android.R.color.darker_gray))
    ...
    private fun setGridMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        //未设置行数 推测行数
        if (mRowNumbers == 0) {
            mRowNumbers = if (childCount % mColumnNumbers == 0) childCount / mColumnNumbers else childCount / mColumnNumbers + 1
        }
        ...
        val tempWidth = (maxWidth + horizontalSpace * (mColumnNumbers - 1) + paddingLeft + paddingRight).toInt()
        val tempHeight = (maxHeight + verticalSpace * (mRowNumbers - 1) + paddingBottom + paddingTop).toInt()
        ...
        setMeasuredDimension(if (modeWidth == MeasureSpec.EXACTLY) sizeWidth else widthResult, if (modeHeight == MeasureSpec.EXACTLY) sizeHeight else heightResult)
    }
...
//                    heightUsed + child.measuredHeight)
    ...
    private fun setGridLayout() {
        ...
        val lp = tempChild
                .layoutParams as MarginLayoutParams
        val childAvWidth = ((sizeWidth - paddingLeft - paddingRight - horizontalSpace * (mColumnNumbers - 1)) / mColumnNumbers).toInt() - lp.leftMargin - lp.rightMargin
        val childAvHeight = ((sizeHeight - paddingTop - paddingBottom - verticalSpace * (mRowNumbers - 1)) / mRowNumbers).toInt() - lp.topMargin - lp.bottomMargin
        for (i in 0 until mRowNumbers) {
            for (j in 0 until mColumnNumbers) {
                val child = getChildAt(i * mColumnNumbers + j)
                if (child != null) {
                    ...
                    if (child.visibility != View.GONE) {
                        ...
                        val childLeft = (paddingLeft + j * (childAvWidth + horizontalSpace)).toInt() + j * (lp.leftMargin + lp.rightMargin) + lp.leftMargin
                        val childTop = (paddingTop + i * (childAvHeight + verticalSpace)).toInt() + i * (lp.topMargin + lp.bottomMargin) + lp.topMargin
    ...
    private fun setChildClickOperation(child: View, key: Int) {
        ...
        child.setOnLongClickListener(OnLongClickListener { view ->
            if (mOnLongItemClickListener != null) {
                mOnLongItemClickListener!!.onLongItemClick((if (key == -1) view.tag else key) as Int, view)
        ...
        child.setOnClickListener(object : OnClickListener {
            override fun onClick(view: View) {
                ...
                if (mOnItemClickListener != null) {
                    mOnItemClickListener!!.onItemClick((if (key == -1) view.tag else key) as Int, view)
                }
            }
        })
    }
            ...
            for (i in 0 until mRowNumbers) {
                for (j in 0 until mColumnNumbers) {
                    ...
                    if (j == mColumnNumbers - 1) {
                        //不是最后一行  只画底部
                        if (i != mRowNumbers - 1) {
                            canvas.drawLine(child.left - horizontalSpace / 2, child.bottom + verticalSpace / 2,
                                    child.right.toFloat(), child.bottom + verticalSpace / 2, linePaint)
                        }
                    } else {
                        //最后一行 只画右部
                        if (i == mRowNumbers - 1) {
                            canvas.drawLine(child.right + horizontalSpace / 2, child.top - verticalSpace / 2,
                                    child.right + horizontalSpace / 2, child.bottom.toFloat(), linePaint)
                        } else {
                            //底部 右部 都画
                            if (j == 0) {
                                canvas.drawLine(child.left.toFloat(), child.bottom + verticalSpace / 2,
                                        child.right + horizontalSpace / 2, child.bottom + verticalSpace / 2, linePaint)
                            } else {
                                canvas.drawLine(child.left - horizontalSpace / 2, child.bottom + verticalSpace / 2,
                                        child.right + horizontalSpace / 2, child.bottom + verticalSpace / 2, linePaint)
                            }
                            if (i == 0) {
                                canvas.drawLine(child.right + horizontalSpace / 2, child.top.toFloat(),
                                        child.right + horizontalSpace / 2, child.bottom + verticalSpace / 2, linePaint)
                            } else {
                                canvas.drawLine(child.right + horizontalSpace / 2, child.top - verticalSpace / 2,
                                        child.right + horizontalSpace / 2, child.bottom + verticalSpace / 2, linePaint)
     ...
     * @param adapter
     */
    fun setAdapter(adapter: com.mooc.resource.widget.flowLayout.FlowAdapter<*>?) {
        mAdapter = adapter as com.mooc.resource.widget.flowLayout.FlowAdapter<T>?
        if (mAdapter!!.count != 0) {

package com.mooc.resource.widget.photoview;
...
public class MPhotoViewContainer  extends FrameLayout {
    ...
    public MPhotoViewContainer(@NonNull Context context) {
        this(context, null);
    }
    public MPhotoViewContainer(@NonNull Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }
    public MPhotoViewContainer(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            switch (ev.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    isHorizantal = (Math.abs(dy) < Math.abs(dx));
                    touchX = ev.getX();
                    touchY = ev.getY();
                    break;
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (isTopOrBottomEnd()  && isHorizantal)return true;
        return result && isHorizantal;
    }
    ...
    ViewDragHelper.Callback cb = new ViewDragHelper.Callback() {
        ...
        public void onViewReleased(@NonNull View releasedChild, float xvel, float yvel) {
            ...
            if (Math.abs(releasedChild.getTop()) > HideTopThreshold) {
                ...
                ViewCompat.postInvalidateOnAnimation(MPhotoViewContainer.this);
            }
        }
    };

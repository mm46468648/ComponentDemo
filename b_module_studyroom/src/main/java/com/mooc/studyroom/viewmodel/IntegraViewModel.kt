...
class IntegraViewModel:BaseListViewModel2<Any>() {
    override suspend fun getData(): Flow<List<Any>?> {
        return flow {
            val await = ApiService.getRetrofit().create(IntegralApi::class.java).getIntegralExchangeList( limit, offset).await()
            emit(await.results)
        }
    }
